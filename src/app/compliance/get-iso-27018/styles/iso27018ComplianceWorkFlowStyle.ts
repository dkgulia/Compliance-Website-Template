import theme from '../../../../theme';

const iso27018ComplianceWorkFlowStyles = {
 box: {
   paddingTop: { sm: '3rem', xs: '1rem', md: '1rem' },
   paddingBottom: { sm: '2rem', xs: '1rem' },
   marginTop: '0rem',
   backgroundColor: theme.palette.background.default,
 },
 workflowOuterContainer: {
   display: 'flex',
   justifyContent: 'center',
   alignItems: 'center',
   width: '100%',
   paddingX: { xs: '1rem', sm: '2rem' },
 },
 workflowContainer: {
   display: 'flex',
   flexDirection: 'column' as const,
   alignItems: 'center',
   justifyContent: 'center',
   textAlign: 'center' as const,
   backgroundColor: '#17171799',
   borderRadius: { xs: '0.75rem', sm: '2rem' },
   width: '100%',
   maxWidth: '72rem',
   marginX: 'auto',
   padding: { xs: '1rem', sm: '3rem 2rem' },
   border: `1px solid ${theme.palette.divider}`,
   boxShadow: theme.customShadows?.card || '0 4px 12px rgba(0, 0, 0, 0.4)',
 },
 gridWrapper: {
   display: 'flex',
   alignItems: 'center',
   justifyContent: 'space-between',
   width: '100%',
 },
 leftSection: {
   display: 'flex',
   flexDirection: 'column' as const,
   alignItems: 'flex-start',
   justifyContent: 'center',
   textAlign: 'left' as const,
   gap: { xs: '0.5rem', sm: '1rem' },
 },
 rightSection: {
   display: 'flex',
   alignItems: 'center',
   justifyContent: 'center',
   textAlign: 'left' as const,
 },
 chip: {
   background: theme.palette.secondary.light,
   color: 'white',
   fontWeight: 'bold',
   borderRadius: '1rem',
   padding: { xs: '0.25rem 0.75rem', sm: '0.4rem 1rem' },
   boxShadow: '0rem 0.25rem 0.375rem rgba(0, 0, 0, 0.3)',
   fontSize: { xs: '0.8rem', sm: '0.9rem' },
 },
 title: {
   fontWeight: '600',
   color: theme.palette.text.primary,
   fontSize: { xs: '1.4rem', sm: '2rem' },
 },
 description: {
   color: '#6B7280',
   fontSize: { xs: '0.85rem', md: '1.1rem' },
   lineHeight: '1.6',
   maxWidth: '100%',
 },
 gridContainer: {
   width: '100%',
   display: 'flex',
   justifyContent: 'center',
   alignItems: { xs: 'center', sm: 'stretch' },
   flexDirection: { xs: 'column', sm: 'row' },
   flexWrap: 'wrap' as const,
   paddingTop: { xs: '1rem', sm: '2rem' },
   gap: { xs: '0.75rem', sm: '1rem' },
 },
 card: {
   flex: '1 1 auto',
   minWidth: { xs: '90%', sm: '12rem' },
   maxWidth: { xs: '90%', sm: '13rem' },
   display: 'flex',
   flexDirection: 'column' as const,
   justifyContent: 'center',
   alignItems: 'center',
   backgroundColor: '#17171799',
   borderRadius: { xs: '0.75rem', sm: '1rem' },
   boxShadow: theme.customShadows?.card || '0 4px 12px rgba(0, 0, 0, 0.4)',
   padding: { xs: '1rem', sm: '2rem' },
   transition: 'transform 0.3s ease, box-shadow 0.3s ease',
   border: `1px solid ${theme.palette.divider}`,
   margin: { xs: '0 auto', sm: '0' },
   '&:hover': {
     transform: 'scale(1.05)',
     boxShadow: '0rem 0.5rem 1.25rem rgba(94, 234, 212, 0.15)',
     backgroundColor: '#262626',
   },
 },
 cardContent: {
   display: 'flex',
   flexDirection: 'column' as const,
   alignItems: 'center',
   justifyContent: 'center',
   gap: { xs: '0.5rem', sm: '1rem' },
   flex: 1,
 },
 iconContainer: {
   width: { xs: '3.5rem', sm: '3.5rem' },
   height: { xs: '3.5rem', sm: '3.5rem' },
   background: '#115e59',
   borderRadius: '50%',
   display: 'flex',
   justifyContent: 'center',
   alignItems: 'center',
   boxShadow: '0 4px 8px rgba(17, 94, 89, 0.4)',
 },
 icon: {
   fontSize: { xs: '0.8rem', sm: '1.8rem' },
   color: 'white',
 },
 cardTitle: {
   fontSize: { xs: '14px', sm: '1rem' },
   fontWeight: 'bold' as const,
   color: theme.palette.text.primary,
   textAlign: 'center' as const,
 },
};

export default iso27018ComplianceWorkFlowStyles;