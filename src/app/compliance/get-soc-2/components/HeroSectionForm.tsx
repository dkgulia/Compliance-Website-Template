...
const Soc2HeroSectionForm: React.FC<Soc2HeroSectionFormProps> = ({
	...
	const handleClick = () => {
		...
		if (values.fullName && !errors.fullName && values.email && !errors.email) {
			setLoading(true);
			// Code commented out as mentioned in original file
				...
				<Grid container spacing={2}>
					<Grid size={{ xs: 12 }}>
						<FormControl fullWidth error={touched.fullName && Boolean(errors.fullName)}>
							<Typography sx={soc2HeroSectionStyles.formLabel}>Full Name</Typography>
							<OutlinedInput
								id="fullName"
								name="fullName"
					...
					<Grid size={{ xs: 12 }}>
						<FormControl fullWidth error={touched.email && Boolean(errors.email)}>
							<Typography sx={soc2HeroSectionStyles.formLabel}>Work Email</Typography>
							<OutlinedInput
								id="email"
								name="email"
